...
import 'package:fire_demo/screen/home_screen/home_screen.dart';
  ...
  Text get_points(int index) {
    if (widget.value[index] == widget.captain) {
      widget.total_points = widget.total_points +
          (((widget.points[widget.value[index]]) != null)
              ? (2 * (widget.points[widget.value[index]]))
              : 0);
      return (widget.points[widget.value[index]] != null)
          ? Text((2 * (widget.points[widget.value[index]])).toString())
          : Text('0');
    } else if (widget.value[index] == widget.vice_captain) {
      widget.total_points = widget.total_points +
          (((widget.points[widget.value[index]]) != null)
              ? (1.5 * (widget.points[widget.value[index]]))
              : 0);
      return (widget.points[widget.value[index]] != null)
          ? Text((1.5 * (widget.points[widget.value[index]])).toString())
          : Text('0');
    } else {
      widget.total_points = widget.total_points +
          (((widget.points[widget.value[index]]) != null)
              ? (widget.points[widget.value[index]])
              : 0);
      return (widget.points[widget.value[index]] != null)
          ? Text((widget.points[widget.value[index]]).toString())
          : Text('0');
    }
  }
  ...
  CircleAvatar get_image(int index) {
    if (widget.batsmen.contains(widget.value[index])) {
      ...
      return CircleAvatar(
        backgroundColor: Colors.white,
        radius: 20,
        backgroundImage: AssetImage('images/wk.jpg'),
      );
    } else if (widget.allrounders.contains(widget.value[index])) {
      return CircleAvatar(
        backgroundColor: Colors.white,
        radius: 20,
        backgroundImage: AssetImage('images/bat_ball.jpg'),
      );
    } else {
      return CircleAvatar(
        backgroundColor: Colors.white,
            ...
            if (index == 0) {
              return Column(
                children: <Widget>[
                  ...
                  Card(
                    child: ListTile(
                      title: (widget.captain == widget.value[index])
                          ? Text(
                              '${widget.value[index]} (C) ',
                              style: GoogleFonts.mcLaren(fontSize: 15),
                            )
                          : ((widget.vice_captain == widget.value[index])
                              ? Text(
                                  '${widget.value[index]} (VC) ',
                                  style: GoogleFonts.mcLaren(fontSize: 15),
                                )
                              : Text(
                                  widget.value[index],
