...
import 'package:fire_demo/data/data.dart';
...
class Match_contest_screen extends StatefulWidget {
  ...
  Match_contest_screen(
      {required this.total_teams,
      required this.all_teams_ref,
...
class _Match_contest_screenState extends State<Match_contest_screen>
        ...
        for (var value in players) {
          if (value == captain) {
            total_points = total_points +
                (((points[value]) != null) ? (2 * (points[value])) : 0);
          } else if (value == vice) {
            total_points = total_points +
                (((points[value]) != null) ? (1.5 * (points[value])) : 0);
          } else {
            total_points = total_points +
                  ...
                  itemBuilder: (context, index) {
                      ...
                      return Column(
                        children: <Widget>[
                          ListTile(
                            ...
                            trailing:
                                Text('Points', style: GoogleFonts.mcLaren()),
                          ),
                          ListTile(
                            title: Text(all_teams_points[index]['Team_Name'],
                                style: GoogleFonts.mcLaren()),
                            trailing: Text(
                                all_teams_points[index]['Points'].toString(),
                                style: GoogleFonts.mcLaren()),
                            leading: Text(same_rank(index).toString(),
                                style: GoogleFonts.mcLaren()),
                    ...
                    return ListTile(
                      ...
                      trailing: Text(
                          all_teams_points[index]['Points'].toString(),
                          style: GoogleFonts.mcLaren()),
                      leading: (all_teams_points[index - 1]['Points'] ==
                              all_teams_points[index]['Points'])
                          ? Text(same_rank(index).toString(),
                              style: GoogleFonts.mcLaren())
                          : Text(diff_rank().toString(),
                              style: GoogleFonts.mcLaren()),
                      tileColor: (same_rank(index) == 1)
                          ? Color(0xFFFDD835)
                          : Colors.white,
                    );
                  },
                    ...
                    if (index == 0) {
                      return Column(
                        children: <Widget>[
                          ...
                          ListTile(
                            title: Text(my_teams[index]['Team_Name'],
                                style: GoogleFonts.mcLaren()),
                            trailing: Text(my_teams[index]['Points'].toString(),
                                style: GoogleFonts.mcLaren()),
